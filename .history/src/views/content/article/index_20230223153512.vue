<template>
  <div class="article-main">
    ...
    <el-card shadow="neveralwys" class="margin-30">
      <div slot="header">
        <el-button type="primary" size="mini" @click="gouAddArticle">新增</el-button>
      </div>
      <el-table v-loading="loading" :data="articleList" border style="width: 100%" stripe>
        ...
        <el-table-column prop="zip" label="文章是否展示" width="200" align="center">
          <template slot-scope="scope">
            <el-switch v-model="scope.row.isShow" :active-value="1" :inactive-value="0" @change="changeArticleShowStatus(scope.row.isShow, scope.row.id)"> </el-switch>
          </template>
        </el-table-column>
        ...
        <el-table-column fixed="right" width="200" label="操作" align="center">
          <template slot-scope="scope">
            <el-button @click="goEditArticle(scope.row.id)" icon="el-icon-view" type="text" size="small">编辑文章</el-button>
            <el-button type="text" size="small" style="color: red" @click="goDeleteArticle(scope.row.id)">删除</el-button>
          </template>
        </el-table-column>
      </el-table>
      <el-pagination
...
import { findArticles as findArticlesApi, changeShowStatus as changeShowStatusApi, delArticle as delArticleApi } from '@/api/content/article'
...
export default {
  ...
  methods: {
    // 编辑文章
    goEditArticle(id) {
      this.$router.push({ name: 'ArtileDetail', params: { id } })
    },
    // 新增文章
    gouAddArticle() {
      this.$router.push({ name: 'ArtileDetail' })
    },
    // 重置表单
    resetForm() {
      this.start = 1
    ...
    getArticleList() {
      this.loading = true
      let searchObj = filterNullParams(this.searchArticleForm)
      findArticlesApi(this.paginationParams.start, this.paginationParams.limit, searchObj).then((res) => {
        console.log(res)
