      ...
      <el-form :model="article" ref="articleForm" :rules="rules" label-width="120px" size="mini">
        ...
        <el-row :gutter="20">
          <el-col :span="24" :offset="0">
            ...
            <mavon-editor ref="markdownEditor" @imgAdd="$imgAdd" v-if="article.editorType == '1'" v-model="article.content1" />
          </el-col>
        </el-row>
        ...
        <el-row :gutter="20" type="flex" justify="center">
          <el-col :span="4">
            <el-form-item label-width="0px" style="margin-top: 20px">
              <el-button type="primary" @click="addArticle">{{ id ? '保存' : '立即新增' }}</el-button>
            </el-form-item>
          </el-col>
        </el-row>
      </el-form>
...
import { addArticle as addArticleApi, productArticle as productArticleApi, updateArticle as updateArticleApi } from '@/api/content/article'
  ...
  methods: {
    ...
    addArticle() {
      this.$refs.articleForm.validate((valid) => {
        if (valid) {
          if (this.article.content === 0) {
            this.article.content2 = this.article.content1
          } else if (this.article.content === 1) {
            this.article.content2 = this.$refs.markdownEditor.d_render
          } // 富文本转换后的 HTML 和原文一样

          // 获取 Markdown 转换后的 HTML 文本
          // 校验成功
          let api = null
          if (this.id) {
            // 有 ID 就是更新文章
